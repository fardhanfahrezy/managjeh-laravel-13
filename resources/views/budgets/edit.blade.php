<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-2xl font-black text-slate-900 dark:text-white tracking-tight">Edit Budget</h1>
                <p class="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-0.5">Perbarui batas limit budget kategori {{ $budget->category->nama }}.</p>
            </div>
        </div>
    </x-slot>
        ...
        <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-sm p-6 sm:p-8">
            <form method="POST" action="{{ route('budgets.update', $budget) }}" class="space-y-6">
                ...
                <div>
                    ...
                    <x-text-input type="month" id="periode" name="periode" :value="old('periode', $budget->periode)" class="mt-1.5 block w-full font-semibold" required />
                ...
                <div>
                    ...
                    <div class="relative mt-1.5 rounded-xl shadow-xs">
                        ...
                        <x-text-input type="number" step="0.01" id="limit_bulanan" name="limit_bulanan" :value="old('limit_bulanan', $budget->limit_bulanan)" class="block w-full pl-12 font-bold" required />
                    </div>
