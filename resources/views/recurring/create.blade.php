<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <div>
                <h1 class="text-2xl font-black text-slate-900 dark:text-white tracking-tight">Buat Aturan Tagihan Berulang</h1>
                <p class="text-xs sm:text-sm text-slate-500 dark:text-slate-400 mt-0.5">Otomatiskan pencatatan langganan dan tagihan terjadwal.</p>
            </div>
        </div>
    </x-slot>
    ...
    <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="bg-white dark:bg-slate-900 rounded-3xl border border-slate-200/80 dark:border-slate-800 shadow-sm p-6 sm:p-8">
            <form method="POST" action="{{ route('recurring-rules.store') }}" class="space-y-6">
                @csrf

                <!-- Nama Tagihan -->
                <div>
                    <x-input-label for="catatan" value="Nama Tagihan / Keterangan" />
                    <x-text-input id="catatan" name="catatan" type="text" class="mt-1.5 block w-full font-semibold" :value="old('catatan')" required placeholder="Contoh: Langganan Netflix, Internet Indihome, BPJS" />
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <div class="relative mt-1.5 rounded-xl shadow-xs">
                            ...
                            <x-text-input id="jumlah" name="jumlah" type="number" min="1" step="1" class="block w-full pl-12 font-bold" :value="old('jumlah')" required placeholder="Contoh: 186000" />
                        </div>
                    ...
                    <div>
                        ...
                        <select id="frekuensi" name="frekuensi" required class="mt-1.5 block w-full rounded-xl border-slate-300 dark:border-slate-700 dark:bg-slate-900 dark:text-white text-sm font-semibold focus:border-blue-600 focus:ring-2 focus:ring-blue-600">
                            <option value="daily" {{ old('frekuensi') === 'daily' ? 'selected' : '' }}>Setiap Hari (Harian)</option>
                            <option value="weekly" {{ old('frekuensi') === 'weekly' ? 'selected' : '' }}>Setiap Minggu (Mingguan)</option>
                            <option value="monthly" {{ old('frekuensi', 'monthly') === 'monthly' ? 'selected' : '' }}>Setiap Bulan (Bulanan)</option>
                            <option value="yearly" {{ old('frekuensi') === 'yearly' ? 'selected' : '' }}>Setiap Tahun (Tahunan)</option>
                        </select>
                ...
                <div>
                    <x-input-label for="tanggal_berikutnya" value="Jatuh Tempo Pertama / Berikutnya" />
                    <x-text-input id="tanggal_berikutnya" name="tanggal_berikutnya" type="date" class="mt-1.5 block w-full font-semibold" :value="old('tanggal_berikutnya', \Carbon\Carbon::now()->format('Y-m-d'))" required />
                    <x-input-error class="mt-2" :messages="$errors->get('tanggal_berikutnya')" />
                </div>

                <div class="flex items-center gap-2.5">
                    <input id="is_active" name="is_active" type="checkbox" value="1" {{ old('is_active', true) ? 'checked' : '' }} class="rounded-md border-slate-300 dark:border-slate-700 dark:bg-slate-900 text-blue-600 focus:ring-blue-600">
