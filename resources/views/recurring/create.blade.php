    ...
    <x-slot name="header">
        <div class="flex items-center gap-2">
            ...
            <h2 class="font-bold text-xl text-gray-900 dark:text-white leading-tight">
                {{ __('Buat Aturan Tagihan Berulang') }}
            </h2>
        </div>
    </x-slot>
    ...
    <div class="py-8">
        <div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
            <div class="p-6 rounded-2xl bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700 shadow-xs">
                <form method="POST" action="{{ route('recurring-rules.store') }}" class="space-y-5">
                    @csrf

                    <div>
                        <x-input-label for="catatan" value="Nama Tagihan / Keterangan" />
                        <x-text-input id="catatan" name="catatan" type="text" class="mt-1 block w-full" :value="old('catatan')" required placeholder="Contoh: Langganan Netflix, Internet Indihome, BPJS" />
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <x-text-input id="jumlah" name="jumlah" type="number" min="1" step="1" class="mt-1 block w-full" :value="old('jumlah')" required placeholder="Contoh: 186000" />
                    ...
                    <div>
                        ...
                        <x-text-input id="tanggal_berikutnya" name="tanggal_berikutnya" type="date" class="mt-1 block w-full" :value="old('tanggal_berikutnya', \Carbon\Carbon::now()->format('Y-m-d'))" required />
                        <x-input-error class="mt-2" :messages="$errors->get('tanggal_berikutnya')" />
                    </div>

                    <div class="flex items-center gap-2">
                        <input id="is_active" name="is_active" type="checkbox" value="1" {{ old('is_active', true) ? 'checked' : '' }} class="rounded border-gray-300 text-emerald-600 focus:ring-emerald-500">
                    ...
                    <div class="flex items-center justify-end gap-3 pt-4 border-t border-gray-100 dark:border-gray-700">
                        ...
                        <x-primary-button>Simpan Aturan</x-primary-button>
                    </div>
                </form>
            </div>
        </div>
