    ...
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            ...
            <div class="flex items-center gap-2">
                ...
                <!-- Import CSV Button -->
                <button type="button" @click="$dispatch('open-modal', 'import-csv-modal')" class="inline-flex items-center gap-1.5 px-3.5 py-2 bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-750 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 text-xs font-bold rounded-xl shadow-xs transition">
                ...
                <!-- Import Modal using x-modal -->
                <x-modal name="import-csv-modal" focusable>
                    <div class="p-6 sm:p-7 space-y-4">
                        ...
                        <p class="text-xs text-slate-500 dark:text-slate-400 leading-relaxed">
                            Unggah file CSV dengan header: <code class="bg-slate-100 dark:bg-slate-800 px-1 py-0.5 rounded text-blue-600 dark:text-blue-400 font-bold">Tanggal, Akun, Tipe, Kategori, Jumlah, Catatan</code>.
                        ...
                        <form method="POST" action="{{ route('transactions.import') }}" enctype="multipart/form-data" class="space-y-4">
                            @csrf
                            <div>
                                <x-input-label value="Pilih File CSV" class="mb-1.5" />
                            ...
                            <div class="flex items-center justify-end gap-2 pt-3 border-t border-slate-100 dark:border-slate-800">
                                ...
                                <x-primary-button>Mulai Import</x-primary-button>
                            </div>
                        </form>
                    </div>
                </x-modal>
                            ...
                            <tr class="hover:bg-slate-50/60 dark:hover:bg-slate-800/40 transition">
                                ...
                                <td class="py-4 px-4 sm:px-6 whitespace-nowrap text-center">
                                    <div class="flex items-center justify-center gap-1.5">
                                        @if($tx->tipe !== 'saving')
                                            <a href="{{ route('transactions.edit', $tx) }}" aria-label="Edit transaksi" class="p-1.5 text-slate-400 hover:text-slate-700 dark:hover:text-white rounded-lg transition" title="Edit Transaksi">
                                        ...
                                        <form method="POST" action="{{ route('transactions.destroy', $tx) }}" onsubmit="return confirm('Apakah Anda yakin ingin menghapus transaksi ini? Saldo akun terkait akan disesuaikan kembali.');">
                                            @csrf
                                            @method('DELETE')
                                            <button type="submit" aria-label="Hapus transaksi" class="p-1.5 text-red-400 hover:text-red-600 rounded-lg transition" title="Hapus Transaksi">
                                                <svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
